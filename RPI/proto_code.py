...
camera1_topic = "rpi/cam/chassis"
camera2_topic = "rpi/cam/cannon"
...
def magnetometer_reading():
    try:
        while True:
            ...
            uartload = "Heading: {:.2f} degrees, Direction: {}\n".format(heading, direction)
            uartload += "Accelerometer: X={:.2f}, Y={:.2f}, Z={:.2f}\n".format(accel[0], accel[1], accel[2])
            uartload += "Gyroscope: X={:.2f}, Y={:.2f}, Z={:.2f}\n".format(gyro[0], gyro[1], gyro[2])

            #print(payload)
            ser.write(uartload.encode('utf-8'))
            sleep(1)
...
def camera_streaming():
    try:
        while True:
            capture_and_send_image(picam2_1, camera1_topic)
            capture_and_send_image(picam2_2, camera2_topic)
            sleep(0.2)
