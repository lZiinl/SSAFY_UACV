...
def predict_gunshot(y, model, transform, classes, sr=44100):
    ...
    with torch.no_grad():
        outputs = model(image)
        probabilities = torch.nn.functional.softmax(outputs, dim=1)  # 확률 계산
        confidence, predicted = torch.max(probabilities, 1)  # 최대 확률 및 클래스 추출
        predicted_class = classes[predicted.item()]
        confidence = confidence.item() * 100  # 확률을 퍼센트로 변환
...
def record_and_predict(duration=1.0, sr=44100, save_path="recordings"):
    ...
    predicted_class, confidence = predict_gunshot(y, model, transform, classes, sr)
